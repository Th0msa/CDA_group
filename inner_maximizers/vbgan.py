...
from assignment3.CDA_group.utils.utils import load_parameters, stack_tensors
from assignment3.CDA_group.utils.script_functions import merge_metrics
from assignment3.CDA_group.datasets.datasets import load_data
import torch.nn.functional as F
# import torchvision
# from torchvision import transforms
# from torchvision.utils import save_image
from assignment3.CDA_group.inner_maximizers.MLP_layer import MLPLayer
...
parser.add_argument('-batch_size', type=int, default= 8, metavar='N', help='input batch size for training (default: 100)')
...
parser.add_argument('-n_input', type=int , default= 22761)
...
train_dataloader_dict, valid_dataloader_dict, test_dataloader_dict, num_features = load_data(
    parameters)
train_loader = train_dataloader_dict['malicious']
test_loader = test_dataloader_dict['malicious']
...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        self.dec5_act = nn.Tanh()

    def encode(self, x):
        h1 = self.enc1.forward(x)
        h2 = self.bn1(h1)
        h3 = self.enc1_act.forward(h2)
        h4 = self.enc2.forward(h3)
        # h5 = self.bn2.forward(h4)
        h6 = self.enc2_act.forward(h4)
        h7 = self.enc3.forward(h6)
        # h8 = self.bn3.forward(h1)
        h9 = self.enc3_act.forward(h7)
        # h = self.enc1_act.forward(self.bn1.forward(self.enc1.forward(x)))
        # h = self.enc2_act.forward(self.bn2.forward(self.enc2.forward(h)))
        # h1 = self.enc3.forward(h)
        # # h2 = self.bn3.forward(h1)
        # h3 = self.enc3_act.forward(h1)
        # h = self.enc3_act.forward(self.bn3.forward(self.enc3.forward(h)))
        # , self.dec5_act.forward(self.enc5.forward(h3))
        return self.dec4_act.forward(self.enc4.forward(h9))

    def reparameterize(self, mu, log_var):
        # std = torch.exp(log_var / 2)
        log_var = torch.exp(log_var / 2)
...
class Decoder(nn.Module):
    ...
    def decode(self, z):
        h = self.dec1_act.forward(self.bn1.forward(self.dec1.forward(z)))
        h = self.dec2_act.forward(self.bn2.forward(self.dec2.forward(h)))
        h = self.dec3_act.forward(self.bn3.forward(self.dec3.forward(h)))
        return self.dec4_act.forward(self.dec4.forward(h))
...
def start():
    ...
    for epoch in range(args.epochs):
        ...
        for i, ((bon_x, bon_y), (mal_x, mal_y)) in enumerate(
            ...
            print("mal_x ", mal_x.shape)
            # x = mal_x.to(device).view(-1, args.n_input)
            x = mal_x.to(device)
            print("x ", x.shape)
            z = torch.randn(args.batch_size, args.n_z).to(device)  # z~N(0,1)
